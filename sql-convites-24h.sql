-- ===== ATUALIZAR SISTEMA DE CONVITES PARA 24 HORAS =====

-- 1. Atualizar função para expirar em 24 horas ao invés de 7 dias
CREATE OR REPLACE FUNCTION public.admin_create_user_with_email(
  p_email TEXT,
  p_full_name TEXT,
  p_group_name TEXT DEFAULT 'user',
  p_department_id UUID DEFAULT NULL,
  p_position_id UUID DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
AS $$
DECLARE
  target_company_id UUID;
  company_name TEXT;
  invite_link TEXT;
  result jsonb;
  email_response jsonb;
  project_url TEXT;
BEGIN
  -- Verificar permissões
  IF NOT (public.is_company_admin() OR public.is_super_admin()) THEN
    RAISE EXCEPTION 'Access denied: admin privileges required';
  END IF;
  
  -- Obter company_id e nome da empresa
  SELECT id, name INTO target_company_id, company_name
  FROM public.companies 
  WHERE id = public.get_user_company_id();
  
  IF target_company_id IS NULL THEN
    RAISE EXCEPTION 'Company ID is required';
  END IF;

  -- Verificar se email já existe
  IF EXISTS (SELECT 1 FROM public.profiles WHERE email = p_email) THEN
    RAISE EXCEPTION 'Email already exists in profiles: %', p_email;
  END IF;

  IF EXISTS (SELECT 1 FROM public.user_invitations WHERE email = p_email AND status IN ('pending', 'email_sent')) THEN
    RAISE EXCEPTION 'Pending invitation already exists for email: %', p_email;
  END IF;

  -- Criar convite com expiração de 24 horas
  INSERT INTO public.user_invitations (
    email, full_name, group_name, company_id, department_id, position_id,
    status, invited_by, expires_at
  ) VALUES (
    p_email, p_full_name, p_group_name, target_company_id, 
    p_department_id, p_position_id, 'pending', auth.uid(), NOW() + INTERVAL '24 hours'
  );

  -- Definir URLs baseadas no ambiente
  project_url := 'https://hklknoybvonvzwfjvqjl.supabase.co';
  invite_link := 'https://cloudflow-vault.vercel.app/complete-signup?email=' || p_email;

  -- Tentar chamar Edge Function para enviar email
  BEGIN
    SELECT content::jsonb INTO email_response
    FROM http_post(
      project_url || '/functions/v1/send-invitation-email',
      json_build_object(
        'email', p_email,
        'fullName', p_full_name,
        'companyName', company_name,
        'inviteLink', invite_link
      )::text,
      'application/json',
      ARRAY[
        http_header('Authorization', 'Bearer ' || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhrbGtub3lidm9udnp3Zmp2cWpsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzAyMjc0MDEsImV4cCI6MjA0NTgwMzQwMX0.WXJFR3iWKPv8T0YZNaSDx5dvGV9Q4w-gAJzfHgNGOA0'),
        http_header('Content-Type', 'application/json')
      ]
    );
    
    -- Verificar se o email foi enviado com sucesso
    IF email_response->>'success' = 'true' THEN
      -- Atualizar status do convite para indicar que o email foi enviado
      UPDATE public.user_invitations 
      SET status = 'email_sent'
      WHERE email = p_email AND status = 'pending';
      
      result := jsonb_build_object(
        'success', true,
        'email', p_email,
        'full_name', p_full_name,
        'message', 'Usuário criado e email de convite enviado com sucesso!',
        'email_sent', true,
        'status', 'email_sent'
      );
    ELSE
      -- Email falhou, mas convite foi criado
      result := jsonb_build_object(
        'success', true,
        'email', p_email,
        'full_name', p_full_name,
        'message', 'Usuário criado, mas falha ao enviar email. Link: ' || invite_link,
        'email_sent', false,
        'invite_link', invite_link,
        'status', 'pending',
        'email_error', email_response->>'error'
      );
    END IF;
    
  EXCEPTION WHEN OTHERS THEN
    -- Se falhar completamente, ainda criar o convite
    result := jsonb_build_object(
      'success', true,
      'email', p_email,
      'full_name', p_full_name,
      'message', 'Usuário criado, mas falha ao enviar email. Link: ' || invite_link,
      'email_sent', false,
      'invite_link', invite_link,
      'status', 'pending',
      'email_error', SQLERRM
    );
  END;

  RETURN result;

EXCEPTION WHEN OTHERS THEN
  RAISE EXCEPTION 'Error creating user invitation: %', SQLERRM;
END;
$$;

-- 2. Função para deletar convite pendente
CREATE OR REPLACE FUNCTION public.delete_invitation(
  p_invitation_id UUID
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
AS $$
DECLARE
  invitation_record RECORD;
BEGIN
  -- Verificar permissões
  IF NOT (public.is_company_admin() OR public.is_super_admin()) THEN
    RAISE EXCEPTION 'Access denied: admin privileges required';
  END IF;
  
  -- Buscar convite para verificar se pertence à empresa do usuário
  SELECT * INTO invitation_record
  FROM public.user_invitations 
  WHERE id = p_invitation_id 
    AND company_id = public.get_user_company_id()
    AND status IN ('pending', 'email_sent');
    
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Invitation not found or already used';
  END IF;
  
  -- Deletar o convite
  DELETE FROM public.user_invitations 
  WHERE id = p_invitation_id 
    AND company_id = public.get_user_company_id();
  
  RETURN jsonb_build_object(
    'success', true,
    'message', 'Convite excluído com sucesso',
    'deleted_email', invitation_record.email
  );

EXCEPTION WHEN OTHERS THEN
  RAISE EXCEPTION 'Error deleting invitation: %', SQLERRM;
END;
$$;

-- 3. Função para limpeza automática de convites expirados
CREATE OR REPLACE FUNCTION public.cleanup_expired_invitations()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
AS $$
DECLARE
  deleted_count INTEGER;
BEGIN
  -- Deletar convites expirados
  DELETE FROM public.user_invitations 
  WHERE expires_at < NOW() 
    AND status IN ('pending', 'email_sent');
  
  GET DIAGNOSTICS deleted_count = ROW_COUNT;
  
  RETURN jsonb_build_object(
    'success', true,
    'deleted_count', deleted_count,
    'message', 'Limpeza automática executada: ' || deleted_count || ' convites expirados removidos'
  );

EXCEPTION WHEN OTHERS THEN
  RAISE EXCEPTION 'Error cleaning up expired invitations: %', SQLERRM;
END;
$$;

-- 4. Criar função que roda automaticamente (trigger ou cron job)
-- Nota: Para funcionar automaticamente, você precisaria configurar um cron job
-- Por enquanto, pode ser chamada manualmente ou via webhook

-- 5. Permissões
GRANT EXECUTE ON FUNCTION public.delete_invitation TO authenticated;
GRANT EXECUTE ON FUNCTION public.cleanup_expired_invitations TO authenticated;

-- 6. Atualizar convites existentes para 24h (opcional)
-- Descomente se quiser atualizar convites já criados
-- UPDATE public.user_invitations 
-- SET expires_at = created_at + INTERVAL '24 hours'
-- WHERE status IN ('pending', 'email_sent') AND expires_at > NOW();

-- 7. Comentários
COMMENT ON FUNCTION public.delete_invitation IS 'Deletes a pending invitation by ID';
COMMENT ON FUNCTION public.cleanup_expired_invitations IS 'Automatically removes expired invitations (24h+)';

-- ===== INSTRUÇÕES =====
/*
FUNCIONALIDADES ADICIONADAS:
1. ✅ Convites expiram em 24 horas ao invés de 7 dias
2. ✅ Função delete_invitation() para excluir convites
3. ✅ Função cleanup_expired_invitations() para limpeza automática
4. ✅ Permissões adequadas para admin/super_admin

PRÓXIMOS PASSOS:
1. Execute este SQL no Supabase
2. O frontend será atualizado para mostrar botão de deletar
3. Para limpeza automática, configure um cron job ou webhook
*/
