-- Fix Critical RBAC Vulnerability
...
WITH CHECK (
  auth.uid() = user_id 
  AND OLD.group_name = NEW.group_name  -- Prevent group_name changes
  AND OLD.status = NEW.status          -- Prevent status changes
);
...
AS $$
BEGIN
  -- Log group_name or status changes
  IF OLD.group_name != NEW.group_name OR OLD.status != NEW.status THEN
    PERFORM public.log_security_event(
      'profile_privilege_change',
      'profiles',
      NEW.id,
      jsonb_build_object('group_name', OLD.group_name, 'status', OLD.status),
      jsonb_build_object('group_name', NEW.group_name, 'status', NEW.status)
    );
  END IF;
