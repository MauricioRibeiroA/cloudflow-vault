...
serve(async (req) => {
  ...
  try {
    ...
    const { S3Client, PutObjectCommand } = await import("https://esm.sh/@aws-sdk/client-s3@3.445.0?target=deno");
    const { getSignedUrl } = await import("https://esm.sh/@aws-sdk/s3-request-presigner@3.445.0?target=deno");
