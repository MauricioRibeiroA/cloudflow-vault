-- ==================================================
-- CORREÇÃO: PROBLEMA DE CONEXÃO - VERSÃO CORRIGIDA
-- ==================================================
-- Remove exceção inexistente "http_request_failed"
-- ==================================================
...
-- 2. FUNÇÃO CORRIGIDA SEM EXCEÇÕES INVÁLIDAS
-- ==================================================
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Capturar todos os erros (incluindo HTTP)
    result := jsonb_build_object(
      'success', true,
      'email', p_email,
      'full_name', p_full_name,
...
-- 4. FUNÇÃO DE TESTE PARA DEBUG (CORRIGIDA)
-- ==================================================
...
-- 5. VERIFICAR SE A EXTENSÃO HTTP ESTÁ DISPONÍVEL
-- ==================================================
...
BEGIN
  -- Verificar se a extensão http existe
  SELECT EXISTS(
  ...
  IF ext_available THEN
    RETURN jsonb_build_object(
      'http_extension_available', true,
      'status', 'OK',
      'message', 'Extensão HTTP está disponível'
    );
  ELSE
    RETURN jsonb_build_object(
      'http_extension_available', false,
      'status', 'ERROR',
      'message', 'Extensão HTTP não está disponível no Supabase'
    );
  END IF;
END;
$$;
...
-- 1. Verificar se extensão HTTP está disponível:
-- SELECT public.check_http_extension();

-- 2. Testar conexão com Edge Function:
-- SELECT public.test_edge_function_connection();

-- 3. Se ambos passarem, testar criação de usuário:
-- SELECT public.admin_create_user_with_email('teste@exemplo.com', 'Usuario Teste', 'user');

-- 4. Verificar logs no Supabase Dashboard > Edge Functions > Logs
