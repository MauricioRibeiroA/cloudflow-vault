-- =====================================================
-- COMPREHENSIVE SQL SCRIPT FOR LOVABLE SUPABASE
...
-- Insert the trial plan with correct column names
INSERT INTO plans (id, name, price_brl, storage_limit_gb, download_limit_gb, max_users, description, stripe_price_id, features)
VALUES ('trial-plan-id', 'Trial Gratuito', 0.00, 1, 5, 3, 'Plano de teste gratuito por 7 dias', NULL, 
        '["Até 1GB de armazenamento", "Até 5GB de download por mês", "Até 3 usuários", "Suporte por email"]'::jsonb)
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'plan_id') THEN
        ALTER TABLE public.companies ADD COLUMN plan_id VARCHAR(255) REFERENCES plans(id);
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'stripe_customer_id') THEN
        ALTER TABLE public.companies ADD COLUMN stripe_customer_id VARCHAR(255);
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'stripe_subscription_id') THEN
        ALTER TABLE public.companies ADD COLUMN stripe_subscription_id VARCHAR(255);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'current_storage_used_bytes') THEN
        ALTER TABLE public.companies ADD COLUMN current_storage_used_bytes BIGINT DEFAULT 0;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'current_download_used_bytes') THEN
        ALTER TABLE public.companies ADD COLUMN current_download_used_bytes BIGINT DEFAULT 0;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'companies' AND column_name = 'active_users_count') THEN
        ALTER TABLE public.companies ADD COLUMN active_users_count INTEGER DEFAULT 0;
    END IF;
END $$;
...
-- Function to start free trial (CORRECTED)
CREATE OR REPLACE FUNCTION start_free_trial(p_company_id UUID)
RETURNS JSON AS $$
DECLARE
    trial_plan_id VARCHAR(255) := 'trial-plan-id';
    result JSON;
BEGIN
...
-- View for self-registered companies (CORRECTED column names)
