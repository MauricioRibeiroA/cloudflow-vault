...
-- REMOVER FUNÇÕES ANTIGAS PRIMEIRO (para evitar conflitos)
...
WHERE id NOT IN (
    ...
    GROUP BY cnpj 
    HAVING cnpj IS NOT NULL AND cnpj != ''
);
...
WHERE user_id NOT IN (
    ...
    GROUP BY cpf 
    HAVING cpf IS NOT NULL AND cpf != ''
);
...
-- 4. ADICIONAR COLUNA ROLE_TYPE se não existir
...
-- 5. FUNÇÃO SEGURA PARA REGISTRO COM VALIDAÇÕES ROBUSTAS
...
BEGIN
    ...
    -- PROTEÇÃO 1: Verificar se CNPJ já existe (múltiplas variações)
    IF EXISTS (
        SELECT 1 FROM public.companies 
        WHERE cnpj = p_cnpj 
        OR cnpj = clean_cnpj
        OR REGEXP_REPLACE(COALESCE(cnpj, ''), '[^0-9]', '', 'g') = clean_cnpj
    ) THEN
    ...
    IF EXISTS (
        ...
        OR cpf = clean_cpf
        OR REGEXP_REPLACE(COALESCE(cpf, ''), '[^0-9]', '', 'g') = clean_cpf
    ) THEN
        RETURN json_build_object(
            'success', false, 
...
-- 6. FUNÇÃO SEGURA PARA COMPLETAR REGISTRO - GARANTIR COMPANY_ADMIN
...
BEGIN
    ...
    INSERT INTO public.profiles (
        ...
        telefone, 
        cargo,
        role_type,        -- CORREÇÃO CRÍTICA: Role correto
        group_name,       -- Mantido para compatibilidade
        is_company_founder,
        is_active,
        can_manage_users  -- Permitir gerenciar usuários
    ) VALUES (
        p_user_id, 
        p_company_id, 
        TRIM(p_admin_name), 
...
-- 7. FUNÇÃO PARA VERIFICAR STATUS DE ADMIN
...
WHERE group_name = 'company_admin' 
   AND (role_type IS NULL OR role_type = 'employee');
